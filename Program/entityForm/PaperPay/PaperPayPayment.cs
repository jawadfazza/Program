...
{
    public partial class PaperPayPayment : Form
    {
        public PaperPayPayment()
        {
            InitializeComponent();
            getTable();
        }
                ...
                if (totalPriceValue > 0&&cbTypeOperation.Text=="كاش")
                 {
                     ...
                     bc.bondFrom ="سند دفع." + txtName.Text.Split('.')[0] + "." + txtid.Text;
                     ...
                     {
                         bc.comment = "تقديم مبلغ نقدي ل  " + txtName.Text + " مقابل تسديد سند الدفع رقم :"+txtid.Text ;
                     }                     
                     //bcList.Add(bc);
                 ...
                 if (totalPriceValue > 0 && cbTypeOperation.Text == "شيك")
                 {
                     ...
                     bc.balance = totalPriceValue;
                     bc.balanceText = nte.changeNumericToWords(totalPriceValue);
                     bc.bondFrom = "سند دفع." + txtName.Text.Split('.')[0].ToString() + "." + txtid.Text;
                     ...
                     {
                         bc.comment = "تقديم شيك بنكي من  " + txtName.Text + " مقابل تسديد سند الدفع رقم :" + txtid.Text;
                     }
                     //bcList.Add(bc);
                     dataGridViewBond.Rows[0].Cells[0].Value = bc.balance;
                     dataGridViewBond.Rows[0].Cells[1].Value = bc.bondFrom;
                 ...
                 IsBondTrested = CheckBondsError("from", 0);
        ...
        private BondControllar setBond2()
        {
            if (totalDiscountValue > 0)
            {
                ...
                bc.bondFrom = "سند دفع." + txtName.Text.Split('.')[0].ToString() + "." + txtid.Text;
                bc.bondTo =  "حسم مكتسب";
                bc.comment = "الحصول على حسم من " + txtName.Text + " لقاء دفعة نقدية";
           ...
           IsBondTrested = CheckBondsError("from", 1);
            return bc;
        }
                ...
                if ((totalDiscountValue + totalPriceValue) <= Convert.ToInt32(txtBalance.Text))
                {
                    if (MessageBox.Show("هل تريد حفط التقرير ؟", "رسالة تأكيد", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
                    {
                        ...
                        if (MessageBox.Show("هل تريد استعراض أمر الدفع ؟", "رسالة تأكيد", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
                        {
                            if (cbTypeOperation.Text == "كاش")
                            {
                                PayBills rp = new PayBills();
                                rp.ShowDialog();
                            }
                            if (cbTypeOperation.Text == "شيك")
                            {
                                PayCheck rc = new PayCheck();
                                rc.ShowDialog();
                            }
                        }
                    }
        ...
        private void bAddCustomer_Click(object sender, EventArgs e)
        {
            ...
            paper_payTableAdapter prta = new paper_payTableAdapter();
            entity.controllar.PaperPayControllar.paper_payDataTable pcdt = prta.getPaperPayById(id);
            PaperPayControllar.paper_payRow prr = pcdt[0];
